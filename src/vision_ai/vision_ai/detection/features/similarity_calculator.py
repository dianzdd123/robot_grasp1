...
# 在 similarity_calculator.py 中添加
    def get_adaptive_feature_weights(self, distance_3d: float, confidence_history: List[float] = None, 
                                   lighting_change: float = 0.0) -> Dict[str, float]:
        """根据距离和历史置信度动态调整特征权重"""
        try:
            print(f"[ADAPTIVE_WEIGHTS] 计算自适应权重: 距离={distance_3d:.1f}mm")
            
            base_weights = self.feature_weights.copy()
            
            # 🆕 距离越近，空间特征权重越高，外观特征权重越低（防止遮挡影响）
            if distance_3d < 50:  # 非常近，可能有遮挡
                base_weights['spatial'] = 0.4      # 大幅提高空间权重
                base_weights['geometric'] = 0.35   
                base_weights['appearance'] = 0.1   # 降低外观权重
                base_weights['shape'] = 0.15
                print(f"[ADAPTIVE_WEIGHTS] 使用近距离权重配置")
            elif distance_3d < 100:  # 较近
                base_weights['spatial'] = 0.25
                base_weights['geometric'] = 0.4
                base_weights['appearance'] = 0.15
                base_weights['shape'] = 0.2
                print(f"[ADAPTIVE_WEIGHTS] 使用中距离权重配置")
            elif distance_3d < 200:  # 中等距离
                base_weights['spatial'] = 0.15
                base_weights['geometric'] = 0.4
                base_weights['appearance'] = 0.25
                base_weights['shape'] = 0.2
                print(f"[ADAPTIVE_WEIGHTS] 使用标准权重配置")
            else:  # 远距离
                base_weights['spatial'] = 0.1
                base_weights['geometric'] = 0.3
                base_weights['appearance'] = 0.35  # 远距离外观更重要
                base_weights['shape'] = 0.25
                print(f"[ADAPTIVE_WEIGHTS] 使用远距离权重配置")
            
            # 🆕 如果置信度在下降，进一步降低外观权重
            if confidence_history and len(confidence_history) >= 3:
                recent_trend = confidence_history[-1] - confidence_history[-3]
                if recent_trend < -0.1:  # 置信度明显下降
                    appearance_reduction = base_weights['appearance'] * 0.5
                    base_weights['appearance'] *= 0.5  # 外观权重减半
                    
                    # 将减少的权重分配给空间和几何特征
                    base_weights['spatial'] += appearance_reduction * 0.6
                    base_weights['geometric'] += appearance_reduction * 0.4
                    
                    print(f"[ADAPTIVE_WEIGHTS] 检测到置信度下降{recent_trend:.3f}，降低外观权重")
            
            # 🆕 光照变化补偿
            if lighting_change > 0.2:  # 光照变化较大
                appearance_reduction = base_weights['appearance'] * 0.3
                base_weights['appearance'] *= 0.7
                base_weights['geometric'] += appearance_reduction * 0.7
                base_weights['shape'] += appearance_reduction * 0.3
                print(f"[ADAPTIVE_WEIGHTS] 检测到光照变化{lighting_change:.2f}，调整权重")
            ...
            print(f"[ADAPTIVE_WEIGHTS] 最终权重: {base_weights}")
            return base_weights
            
        except Exception as e:
            print(f"[ADAPTIVE_WEIGHTS] 计算自适应权重失败: {e}")
            return self.feature_weights.copy()
    def calculate_detailed_similarity_breakdown_adaptive(self, reference_features: Dict, 
                                                       candidate_features: Dict, 
                                                       waypoint_data: Dict = None,
                                                       tracking_context: Dict = None) -> Dict:
        """计算详细的细粒度特征相似度分解 - 自适应版本"""
        try:
            print(f"[ADAPTIVE_SIMILARITY] 开始自适应相似度计算")
            
            # 🆕 获取追踪上下文
            distance_3d = tracking_context.get('distance_3d', 100.0) if tracking_context else 100.0
            confidence_history = tracking_context.get('confidence_history', []) if tracking_context else []
            lighting_change = tracking_context.get('lighting_change', 0.0) if tracking_context else 0.0
            
            # 🆕 计算自适应权重
            adaptive_weights = self.get_adaptive_feature_weights(distance_3d, confidence_history, lighting_change)
            
            # 使用自适应权重临时替换原权重
            original_weights = self.feature_weights.copy()
            self.feature_weights = adaptive_weights
            ...
            return {
                ...
                'final_score': final_result['final_score'],
                'feature_contributions': final_result['contributions'],
                'adaptive_weights_used': adaptive_weights,  # 🆕 记录使用的自适应权重
                'tracking_context': tracking_context
            }
            
        except Exception as e:
            print(f"[ADAPTIVE_SIMILARITY] 自适应相似度计算失败: {e}")
            # 恢复原权重
            self.feature_weights = original_weights
            return {'detailed_breakdown': {}, 'final_score': 0.0, 'feature_contributions': {}}